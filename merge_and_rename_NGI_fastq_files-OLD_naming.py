...
#    sample_pattern=re.compile("^(P[0-9]+_[0-9]+)_S[0-9]+_.+_R([1-2])_")
    ...
    while fastq_files:
        ...
        for fq in fastq_files:
            if sample_name in os.path.basename(fq) and fq.endswith('_1.fastq.gz'):
                fastq_files_read1.append(fq) 
                
            if sample_name in os.path.basename(fq) and fq.endswith('_2.fastq.gz'):
...
def actual_merging(sample_name, read_nb, tomerge, dest_dir):
    outfile=os.path.join(dest_dir, "{}_{}.fastq.gz".format(sample_name, read_nb))
